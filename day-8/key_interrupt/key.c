...
static struct key_info *ppload =NULL;
/*
   描述按键信息的结构体数组
 */
...
/*
   中断底半部处理函数
*/
static void tasklet_interrupt_fun(unsigned long data)
{
	int state = 0;
	struct key_info *pload=(struct key_info *)ppload;	//将dev_id中断描述强制转换为key_info型结构体
...
static void task_initial_fun(void)
{
	//????/
	tasklet_init(&key_tasklet,tasklet_interrupt_fun,0);
	//DECLARE_TASKLET(key_tasklet,tasklet_interrupt_fun,(unsigned long)ppload);
}
